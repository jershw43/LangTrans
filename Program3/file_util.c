...
/*
    file_util.c - File utility functions
...
void init_temp_files()
{
    ...
    /* Create temp1 as an empty file, then append its contents to the output file */
    file_open(&g_temp1_file, g_temp1_filename, "w", &g_temp1_opened);
    file_close(&g_temp1_file, &g_temp1_opened);
 
    /* Reopen temp1 for reading and append to output */
    file_open(&g_temp1_file, g_temp1_filename, "r", &g_temp1_opened);
    while ((ch = (char)fgetc(g_temp1_file)) != (char)EOF)
    {
        fputc(ch, g_output_file);
    }
    file_close(&g_temp1_file, &g_temp1_opened);
 
    /* Create temp2 for loop information (future use) */
